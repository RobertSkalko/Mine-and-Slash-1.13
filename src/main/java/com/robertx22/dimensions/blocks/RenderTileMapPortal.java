...
public class RenderTileMapPortal extends TileEntityRenderer<TileMapPortal> {
    ...
    public void render(TileEntityEndPortal tileEntityIn, double x, double y, double z, float partialTicks,
	    int destroyStage) {
	...
	for (int j = 0; j < i; ++j) {
	    ...
	    GlStateManager.translatef(17.0F / f2,
		    (2.0F + f2 / 1.5F) * ((float) Util.milliTime() % 800000.0F / 800000.0F), 0.0F);
