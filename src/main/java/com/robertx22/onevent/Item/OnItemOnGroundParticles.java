            ...
            for (EntityItem en : p.world.getEntitiesWithinAABB(EntityItem.class, box)) {
                ...
                if (stack.hasTag() && stack.getTag().hasKey("rarity")) {
                    ...
                    if (rarity == -1) { // if unique
                        ...
                        en.world.addParticle(Particles.CRIT, false, en.posX + rand.nextFloat() * radius - 0.1, en.posY + en.height / 2 + rand
                                .nextFloat() * radius - 0.1, en.posZ + rand.nextFloat() * radius - 0.1, x, y, z);
                    }

                    for (int i = 0; i < amount; i++) {

                        en.world.addParticle((IParticleData) Particles.DUST, false, en.posX + rand
                                .nextFloat() * radius - 0.1, en.posY + en.height / 2 + rand
                                .nextFloat() * (radius + yhigh), en.posZ + rand.nextFloat() * radius - 0.1, x, y, z);
