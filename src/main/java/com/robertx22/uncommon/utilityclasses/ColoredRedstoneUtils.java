...
public class ColoredRedstoneUtils {
    ...
    public static void SpawnNovaRedstone(Elements element, Entity entity, double radius,
                                         int amount) {

        Color color = Color.BY_ELEMENT.get(element);
        String name = new NovaParticleGen().Name();

        ParticleUtils.spawnParticleGenerator(entity, name, entity.posX, entity.posY + entity.height / 2, entity.posZ, color.x, color.y, color.z, radius, amount);
    }

    public static void SpawnAoeRedstone(Elements element, Entity entity, double radius,
                                        int amount) {

        Color color = Color.BY_ELEMENT.get(element);
        String name = new AoeProjectileParticleGen().Name();

        ParticleUtils.spawnParticleGenerator(entity, name, entity.posX, entity.posY, entity.posZ, color.x, color.y, color.z, radius, amount);
    }
